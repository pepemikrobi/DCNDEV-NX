...
resource "null_resource" "dc1_setup_console" {
    ...
    triggers = {
      trigger = "${vsphere_virtual_machine.dc1_nexus[each.key].uuid}"
    }
...
resource "vsphere_virtual_machine" "dc2_nexus" {

  for_each = var.dc2_switch_data
...
resource "null_resource" "dc2_setup_console" {
    for_each = var.dc2_switch_data

    triggers = {
      trigger = "${vsphere_virtual_machine.dc2_nexus[each.key].uuid}"
    }
    provisioner "local-exec" {
